...
public class JDBCExample {
	public static void main(String[] args) {
		...
		try {
			...
			// 3. SQL 작성
			// ** JAVA에서 작성되는 SQL은 마지막에 ;(세미콜론) 찍지 않음 주의 !
			...
			// 3단계 : SQL을 수행해서 반환 받은 결과(ResultSet)을
			//			한 행씩 접근해서 컬럼 값 얻어오기
			...
			while(rs.next()) {
				...
				// java.sql.Date의 toString은 yyyy-mm-dd 형식으로 오버라이딩 되어있음.
			...
			// SQLException : DB 관련 최상위 예외 (== 데이터베이스 관련 모든 예외)
			e.printStackTrace();
		} finally {
